...
let newUsers = users.filter((user, index) => {
  // return true;
  return user.includes("anh"); //mình coi cái này là true
})
console.log(newUsers);

// tìm sản phẩm có id = 2 
let idSearch = "product 1"; // = 2
let searchPro = productsOfVy.filter((pro, _) => {
  return pro.name === idSearch;
});
...
let productMatch = productsOfVy.find((pro) => pro.id !== 1);
console.log(productMatch);

// 6,7 findIndex , findLastIndex ( callback ) => trả về vị trí của thằng muốn tìm
let indexMatch = productsOfVy.findIndex((pro) => pro.id !== 1);
...
let flattenedNumbers = numbers.flatMap(num => num);
...
let loaibo = [...new Set(traiMang)]
console.log(traiMang);
console.log(loaibo);



let bai1 = movies.reduce((mov, genre) => {
  return mov.concat(genre.genres);
},[]);
console.log(bai1);
...
function CheckSpread(a, b, c, ...args) {
  console.log(a, b, c);
  console.log(...args); /// thí dụ 1 2 3 và  4 5
